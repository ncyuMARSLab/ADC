...
char *g_input_folder = "..\\..\\Input\\ADC\\";//10.16.173.110/Public/output_1B_for_65536_class/"; // "..//..//input//";//smae folder for this project
char *g_output_folder = "..\\..\\Output\\";
string g_file_name = "";

unsigned __int8 **g_data_cluster = NEW2D(TABLE_NUM, DATA_NUM, unsigned __int8);
unsigned __int8 **g_residual_data_cluster = NEW2D(TABLE_NUM, DATA_NUM, unsigned __int8);
...
double ***g_residual_codebook = NEW3D(TABLE_NUM, CLUSTER_NUM, DATA_TABLE_DIM, double);
...
void LoadFile() {
	...
	FILE *input_file_residual_codebook[TABLE_NUM];
	FILE *input_file_residual_data_clulste[TABLE_NUM];

	thread load_residual_data_clulste[TABLE_NUM];
	thread load_residual_codebook[TABLE_NUM];
	...
	for (int table_id = 0; table_id < TABLE_NUM; table_id++) {
		...
		g_file_name = "SIFT_codebook_table_" + to_string(table_id) + ".txt"; //每張表分256群後的codebook
		...
		g_file_name = "SIFT_ClassMember_table_" + to_string(table_id) + ".txt"; //每張表中每一群的資料ID
		...
		g_file_name = "Residual_codebook_table_" + to_string(table_id) + ".txt"; //每張表分256群後的codebook
		CheckOpenFile(input_file_residual_codebook[table_id], g_input_folder, g_file_name, "rb");
		load_residual_codebook[table_id] = thread(LoadCodebook, input_file_residual_codebook[table_id], g_residual_codebook[table_id], CLUSTER_NUM);

		g_file_name = "Residual_ClassMember_table_" + to_string(table_id) + ".txt"; //每張表中每一群的資料ID
		CheckOpenFile(input_file_residual_data_clulste[table_id], g_input_folder, g_file_name, "rb");
		load_residual_data_clulste[table_id] = thread(LoadDataCluster, input_file_residual_data_clulste[table_id], g_residual_data_cluster[table_id]);

	}
	for (int table_id = 0; table_id < TABLE_NUM; table_id++) {
		load_codebook[table_id].join();
		load_data_clulste[table_id].join();
		load_residual_codebook[table_id].join();
		load_residual_data_clulste[table_id].join();
		load_query[table_id].join();
		fclose(input_file_query[table_id]);
		fclose(input_file_codebook[table_id]);
		fclose(input_file_data_clulste[table_id]);
		fclose(input_file_residual_codebook[table_id]);
		fclose(input_file_residual_data_clulste[table_id]);
	}
...
inline double Distance(double data[], double codebook[], double residual_codebook[]) {
	double dist = 0;
	double res;
	for (int j = 0; j < DATA_TABLE_DIM; j++) {
		res = data[j] - (codebook[j] + residual_codebook[j]);
		dist += res * res;
	}
	return dist;
}
...
void CalculateTopRecall_thread(string g_file_name, int thread_id, int &hit_sum, double &precision_sum, int &candidate_count_avg, double &time, int recallR) {
	...
	for (int i = 0; i < QUERY_NUM / THREAD_NUM; i++, query_id++) {
		...
		while (CharToIntOnlyAutoStop(&saveptr, &canditate_id)) {
			dist = 0;
			for (int table_id = 0; table_id < TABLE_NUM; table_id++) {
				dist += Distance(g_query[table_id][query_id], g_codebook[table_id][g_data_cluster[table_id][canditate_id]], g_residual_codebook[table_id][g_residual_data_cluster[table_id][canditate_id]]);
			}
...
void Processing(int recallR) {
	/*string file_name[] = { "CandidateID_4096x4096_M6_1.txt",
							"CandidateID_4096x4096_M6_2.txt",
							"CandidateID_4096x4096_M6_3.txt", 
							"CandidateID_4096x4096_M6_4.txt", 
							"CandidateID_4096x4096_M6_5.txt", 
							"CandidateID_4096x4096_M6_6.txt", 
							"CandidateID_4096x4096_M6_7.txt", 
							"CandidateID_4096x4096_M6_8.txt", 
							"CandidateID_4096x4096_M6_9.txt", 
							"CandidateID_4096x4096_M6_10.txt", 
							"CandidateID_4096x4096_M6_11.txt", 
							"CandidateID_4096x4096_M6_12.txt", 
							"CandidateID_4096x4096_M6_13.txt", 
							"CandidateID_4096x4096_M6_14.txt", 
							"CandidateID_4096x4096_M6_15.txt" };*/
	string file_name[] = {
